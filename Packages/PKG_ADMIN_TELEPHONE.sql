CREATE OR REPLACE PACKAGE PKG_ADMIN_TELEPHONE  IS
       ...
       PROCEDURE GET_ALL_TELEPHONE(PRECORDSET OUT SYS_REFCURSOR, PNID_TELEPHONE IN VARCHAR2 default null);
...
CREATE OR REPLACE PACKAGE BODY PKG_ADMIN_TELEPHONE IS
    ...
    PROCEDURE SET_PHONE_NUMBER(PNIDENTIFICATION VARCHAR2, PNNEW_VALUE VARCHAR2) IS
      ...
      PROCEDURE GET_ALL_TELEPHONE(PRECORDSET OUT SYS_REFCURSOR, PNID_TELEPHONE IN VARCHAR2 default null) AS
                VMENERROR        EXCEPTION;
       BEGIN
              OPEN pRecordSet FOR
              SELECT *
              FROM TELEPHONE
              WHERE TELEPHONE = NVL(PNID_TELEPHONE, TELEPHONE);
