CREATE OR REPLACE PACKAGE PKG_ADMIN_EMAIL  IS
      ...
      PROCEDURE GET_ALL_EMAIL(PRECORDSET OUT SYS_REFCURSOR, PNEMAIL IN number default null);
...
/*         PROCEDURE SET_ID_PERSON(PNNEW_VALUE VARCHAR2, PNOLD_VALUE VARCHAR2) IS
      ...
      PROCEDURE GET_ALL_EMAIL(PRECORDSET OUT SYS_REFCURSOR, PNEMAIL IN number default null) AS
                VMENERROR        EXCEPTION;
       BEGIN
              OPEN pRecordSet FOR
              SELECT *
              FROM EMAIL
              WHERE EMAIL = NVL(PNEMAIL, EMAIL);
