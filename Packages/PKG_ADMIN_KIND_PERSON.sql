CREATE OR REPLACE PACKAGE PKG_ADMIN_KIND_PERSON  IS
       ...
       procedure VALIDATE_USER(PNUSERNAME in VARCHAR2, PNPASSWORD in VARCHAR2,PNCONDITION out NUMBER);
...
CREATE OR REPLACE PACKAGE BODY PKG_ADMIN_KIND_PERSON IS
    ...
    PROCEDURE SET_USERTYPE_NAME(PNID_USERTYPE NUMBER, PNNEW_VALUE VARCHAR2) IS
     ...
     procedure VALIDATE_USER(PNUSERNAME in VARCHAR2, PNPASSWORD in VARCHAR2,PNCONDITION out NUMBER)IS
       BEGIN
         SELECT COUNT(*)
         INTO pncondition
         FROM USERNAME
