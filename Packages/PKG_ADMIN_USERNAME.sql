CREATE OR REPLACE PACKAGE PKG_ADMIN_USERNAME IS
       ...
       PROCEDURE SET_PASSWORD(PNIDENTIFICATION VARCHAR2, PNNEW_VALUE VARCHAR2);
       PROCEDURE GET_ALL_USERNAME(PRECORDSET OUT SYS_REFCURSOR, PNID_USERNAME IN VARCHAR2 default null);
       PROCEDURE DELETE_ALL_USERNAME (pnId_USERNAME IN VARCHAR2 DEFAULT NULL);
...
CREATE OR REPLACE PACKAGE BODY PKG_ADMIN_USERNAME IS
    ...
    PROCEDURE SET_IDENTIFICATION(PNIDENTIFICATION VARCHAR2, PNNEW_VALUE VARCHAR2) IS
      VMENERROR                                   EXCEPTION;
         ...
         ---------------------------------------------------------------------------------
    PROCEDURE SET_PASSWORD(PNIDENTIFICATION VARCHAR2, PNNEW_VALUE VARCHAR2) IS
         old_pass varchar2(20);
         VMENERROR    EXCEPTION;
         BEGIN
             select password into old_pass from username where identification=pnidentification;
             ...
             INSERT INTO BINNACLE (ID_BINNACLE,ID_PERSON,PREVIOUS_PASSWORD,CURRENT_PASSWORD,PASS_CHANGE_DATE)
                    VALUES(S_BINNACLE.NEXTVAL,PNIDENTIFICATION,OLD_PASS,PNNEW_VALUE,SYSDATE);            
                          IF SQL%NOTFOUND THEN
                    RAISE VMENERROR;
            END IF;
            EXCEPTION 
      ...
      PROCEDURE GET_ALL_USERNAME(PRECORDSET OUT SYS_REFCURSOR, PNID_USERNAME IN VARCHAR2 default null) AS
       ...
       BEGIN
              OPEN pRecordSet FOR
              SELECT USERNAME,PASSWORD,IDENTIFICATION,ID_USERTYPE
              FROM USERNAME
       ...
       PROCEDURE DELETE_ALL_USERNAME (pnId_USERNAME IN VARCHAR2 DEFAULT NULL) AS
         vmError exception;
         begin
            DELETE FROM USERNAME WHERE IDENTIFICATION = NVL(PNID_USERNAME,IDENTIFICATION);
            IF SQL%NOTFOUND THEN
              RAISE VMERROR;
            END IF;
            EXCEPTION
