...
CREATE OR REPLACE PACKAGE PKG_ADMIN_COUNTRY  IS
       ...
       PROCEDURE GET_ALL_COUNTRY(PRECORDSET OUT SYS_REFCURSOR, PNID_COUNTRY 
       IN number default null);
       PROCEDURE DELETE_ALL_COUNTRY (pnId_COUNTRY IN NUMBER DEFAULT NULL);
...
CREATE OR REPLACE PACKAGE BODY PKG_ADMIN_COUNTRY IS
      ...
      PROCEDURE GET_ALL_COUNTRY(PRECORDSET OUT SYS_REFCURSOR, PNID_COUNTRY IN number default null) AS
       ...
       PROCEDURE DELETE_ALL_COUNTRY (pnId_COUNTRY IN NUMBER DEFAULT NULL) AS
         vmError exception;
         begin
            DELETE FROM COUNTRY WHERE ID_COUNTRY = NVL(PNID_COUNTRY,ID_COUNTRY);
            IF SQL%NOTFOUND THEN
              RAISE VMERROR;
            END IF;
            EXCEPTION
